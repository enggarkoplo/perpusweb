		...
		if (jml > stok) {
			...
			$.ajax({
				...
				success: function (hasil) {
					cols +=
						'<td><input type="hidden" name="idbuku[]" value="' +
						hasil[0].id_buku +
...
function ambilAnggota() {
	...
	if (anggota == "") {
		$("#preview").attr("src", link + "assets/upload/anggota/user.png");
		$("#namaL").text("-");
		$("#jk").text("-");
		// $("#umur").text("-");
		$("#notelp").text("-");
		...
		$.ajax({
			...
			success: function (hasil) {
				$("#preview").attr(
					"src",
					link + "assets/upload/anggota/" + hasil[0].foto
				);
				$("#namaL").text(hasil[0].nama_lengkap);
				$("#jk").text(hasil[0].jk);
				// $("#umur").text(hasil[0].umur);
				$("#notelp").text(hasil[0].notelp);
				$("#kelas").text(hasil[0].kelas);
				$("#jurusan").text(hasil[0].jurusan);
			},
		});
	}
}
